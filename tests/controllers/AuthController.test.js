...
describe('AuthController', () => {
  ...
  describe('GET: /connect', () => {
    it('Failure with no "Authorization" header field', function (done) {
      this.timeout(5000);
      request.get('/connect')
        .expect(401)
        .end((err, res) => {
  ...
  describe('GET: /disconnect', () => {
    ...
    it('Successfully disconnects a user', function (done) {
      this.timeout(5000);
      request.get('/disconnect')
        .set('Authorization', `Bearer ${token}`) // Assuming token is available
        .expect(200)
        .end((err, res) => {
          if (err) {
            return done(err);
          }
          expect(res.body).to.deep.eql({ message: 'Disconnected successfully' });
          done();
        });
    });
  });
