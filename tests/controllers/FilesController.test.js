...
describe('+ FilesController', () => {
  ...
  const mockFiles = [
    ...
    {
      name: 'verse_titles.md',
      type: 'file',
      data: [
        '+ Line 4: Line content 4',
        '+ Line 8: Line content 8',
        '+ Line 9: Line content 9',
        '+ Line 2: Line content 2',
        '+ Line 1: Line content 1',
        '+ Line 7: Line content 7',
        '',
      ].join('\n'),
      b64Data() { return Buffer.from(this.data, 'utf-8').toString('base64'); },
    },
  ...
  describe('+ POST: /files', () => {
    it('+ Failure with no "X-Token" header field', function (done) {
      request.post('/files')
        .expect(401)
        .end((err, res) => {
          if (err) {
    ...
    it('+ Failure for a non-existent user', function (done) {
      this.timeout(5000);
      request.post('/files')
        .set('X-Token', 'raboof')
        .expect(401)
    ...
    it('+ Failure if name is missing', function (done) {
      this.timeout(5000);
      request.post('/files')
        .set('X-Token', token)
        .send({})
    ...
    it('+ Failure if type is missing', function (done) {
      this.timeout(5000);
      request.post('/files')
        .set('X-Token', token)
        .send({ name: 'song_titles.txt' })
        .expect(400)
        .end((requestErr, res) => {
          if (requestErr) {
            return done(requestErr);
    ...
    it('+ Failure if type is available but unrecognized', function (done) {
      this.timeout(5000);
      request.post('/files')
        .set('X-Token', token)
        .send({ name: 'song_titles.txt', type: 'song' })
        .expect(400)
        .end((requestErr, res) => {
          if (requestErr) {
            return done(requestErr);
  ...
  describe('+ PUT: /files/:id/publish', () => {
    it('+ Fails with no "X-Token" header field', function (done) {
      request.put('/files/1145653/publish')
        .expect(401)
        .end((err, res) => {
          if (err) {
            return done(err);
    ...
    it('+ Fails for a non-existent user', function (done) {
      this.timeout(5000);
      request.put('/files/11455653/publish')
        .set('X-Token', 'raboof')
        .expect(401)
        .end((requestErr, res) => {
          if (requestErr) {
  ...
  describe('+ PUT: /files/:id/unpublish', () => {
    it('+ Fails with no "X-Token" header field', function (done) {
      request.put('/files/11455653/unpublish')
        .expect(401)
        .end((err, res) => {
          if (err) {
            return done(err);
  ...
  describe('+ GET: /files/:id/data', () => {
    it('+ Fails if the file does not exist', function (done) {
      request.get('/files/11455653/data')
        .expect(404)
        .end((err, res) => {
          if (err) {
            return done(err);
